...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ checked, onChange, disabled = false }) => {
  return (
    <button
      ...
      onClick={onChange}
      disabled={disabled}
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all duration-200 ease-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-action-primary focus:ring-offset-2 ${
        checked
          ? 'bg-action-primary shadow-lg'
          : 'bg-border-strong shadow-inner border border-border-default'
