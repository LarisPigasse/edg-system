...
const Badge: React.FC<BadgeProps> = ({ children, text, variant, size = "xs", status, className }) => {
  ...
  // 🎨 Variant classes — superfici tenui dai token del tema.
  // Le varianti `dark:` non servono più: i token cambiano da soli col tema.
  const variantClasses = {
    // Stesso fondo e stesso bordo per tutte: a portare il significato è il testo.
    primary: 'border border-surface-border bg-surface-2 text-text-link',
    success: 'border border-surface-border bg-surface-2 text-text-success',
    warning: 'border border-surface-border bg-surface-2 text-text-warning',
    danger: 'border border-surface-border bg-surface-2 text-text-danger',
    info: 'border border-surface-border bg-surface-2 text-text-info',
    default: 'border border-surface-border bg-surface-2 text-text-secondary',
  };
