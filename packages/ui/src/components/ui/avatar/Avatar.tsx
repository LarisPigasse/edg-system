...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const variantConfig = {
    ...
    secondary: "bg-action-neutral text-white",
    success: "bg-action-success text-white",
    warning: "bg-action-warning text-white",
    danger: "bg-action-danger text-white",
    info: "bg-action-info text-white",
  };

  // Status configuration
  const statusConfig = {
    online: "bg-action-success",
    offline: "bg-action-neutral",
    busy: "bg-action-danger",
    away: "bg-action-warning",
  };
  ...
  return (
    <div
      className={cn(
        ...
        isClickable && "focus:outline-none focus:ring-2 focus:ring-action-primary focus:ring-offset-2",
        isClickable && "active:scale-95",
        // Border
        bordered && "ring-2 ring-bg-primary shadow-md",
        // Disabled state
    ...
    >
      ...
      {shouldShowImage && !imageLoaded && (
        <div
          ...
          aria-hidden="true"
        >
          <User className={cn("text-text-placeholder", config.icon)} />
        </div>
      )}
