...
export const StatCard: React.FC<StatCardProps> = ({ title, value, icon, color, loading, alert, subtitle, onClick }) => {
  ...
  return (
    <Card variant='default' padding='md' onClick={onClick} className={alert ? 'border border-surface-border' : ''}>
      <div className='flex items-center justify-between'>
        <div>
          <p className='text-sm font-medium text-text-secondary'>{title}</p>
          <p className={`text-3xl font-bold mt-1 ${alert ? 'text-text-danger' : 'text-text-primary'}`}>
